  ...
  return <main className="bg-background pb-24 pt-32 lg:pt-40">
    ...
    <section className="section-shell mt-10">
      {filtered.length === 0 ? <div className="border-y fine-rule py-16 text-center"><p className="text-lg">{t.planning.empty}</p><Button variant="dark" className="mt-6" onClick={reset}>{t.planning.reset}</Button></div> : <>
        <div className="space-y-10 md:hidden">{visibleDays.map((item) => <DayGroup key={item} day={item} slots={filtered.filter((slot) => slot.jour === item)} names={t.planning.disciplineNames} confirm={t.planning.confirm} current={item === currentScheduleDay()} today={t.planning.today} />)}</div>
...
function DayGroup({ day, slots, names, confirm, current, today }: { day: Jour; slots: Creneau[]; names: Record<string, string>; confirm: string; current: boolean; today: string }) { return <section><h2 className={`border-b-2 pb-3 font-display text-4xl ${current ? "border-primary" : "border-foreground"}`}>{capitalize(day)}{current && <span className="ml-3 align-middle font-sans text-xs font-semibold uppercase text-primary">{today}</span>}</h2><div className="mt-4 grid gap-px bg-border">{slots.map((slot) => <SlotCard key={slot.id} slot={slot} names={names} confirm={confirm} mobile />)}</div></section>; }
