  ...
  return <main>
    <section className="relative flex min-h-[92svh] items-end overflow-hidden bg-surface-deep pb-14 pt-32 text-hero-foreground sm:pb-20 lg:min-h-[94svh] lg:pb-24">
      ...
      <div className="section-shell relative z-10">
        ...
        <h1 className="editorial-title max-w-5xl text-5xl sm:text-7xl lg:text-8xl xl:text-9xl"><span className="hero-title-line">L’ART DE GRANDIR.</span><span className="hero-title-line">LE PLAISIR DE S’EXPRIMER.</span></h1>
    ...
    <section id="disciplines" className="bg-surface-deep py-20 text-hero-foreground lg:py-28"><div className="section-shell"><SectionLabel>{t.disciplines.eyebrow}</SectionLabel><RevealTitle lines={[t.disciplines.title]} className="mt-5 max-w-4xl text-6xl sm:text-7xl lg:text-9xl" /><div className="mt-12 grid auto-rows-[15rem] grid-cols-1 gap-px bg-hero-foreground/15 sm:grid-cols-2 lg:grid-cols-4">
      {universeItems.map((item, index) => { const media = disciplineMedia[item.slug as keyof typeof disciplineMedia]?.[0]; const mediaText = t.media[item.slug as keyof typeof t.media]?.[0]; return <Reveal key={item.slug} index={index} className={`${index === 0 || index === 7 ? "lg:col-span-2" : ""} ${index === 2 ? "lg:row-span-2" : ""}`}><Link to={item.href} className="discipline-card group relative flex size-full overflow-hidden bg-surface-dark p-6">{media && <img src={media} alt={mediaText?.alt ?? item.name} className="discipline-card-media absolute inset-0 size-full object-cover" />}<span className="discipline-card-overlay absolute inset-0 bg-surface-deep" /><span className="relative ml-auto text-xs text-hero-foreground/65">{String(index + 1).padStart(2,"0")}</span><div className="relative mt-auto"><p className="text-xs uppercase text-primary">{media ? t.brand.name : t.disciplines.placeholder}</p><h3 className="discipline-card-name editorial-title mt-2 text-4xl sm:text-5xl">{item.name}</h3><span className="mt-5 inline-flex items-center gap-2 text-xs font-semibold uppercase">{t.disciplines.discover}<ArrowRight className="size-4" /></span><span className="discipline-card-rule mt-4 block h-0.5 w-full bg-primary" /></div></Link></Reveal>; })}
    </div></div></section>

    <section className="bg-background py-20 lg:py-28"><div className="section-shell"><SectionLabel>{t.audiences.eyebrow}</SectionLabel><h2 className="editorial-title mt-5 max-w-3xl text-5xl sm:text-7xl">{t.audiences.title}</h2><div className="mt-12 grid border-l border-t fine-rule sm:grid-cols-2 lg:grid-cols-4">{t.audiences.items.map((item, index) => <a key={item} href="#disciplines" className="group flex min-h-56 flex-col justify-between border-b border-r fine-rule p-6 hover:bg-primary"><span className="text-xs text-muted-foreground">0{index+1}</span><div><h3 className="font-display text-4xl">{item}</h3><p className="mt-3 flex items-center gap-2 text-xs font-semibold uppercase">{t.audiences.discover}<ArrowRight className="size-4" /></p></div></a>)}</div></div></section>

    <section className="bg-muted py-20 lg:py-28"><div className="section-shell grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-center lg:gap-20"><RevealImage src={signAsset.url} alt={t.about.imageAlt} className="aspect-[4/3] lg:order-2" /><Reveal><SectionLabel>{t.about.eyebrow}</SectionLabel><RevealTitle lines={["PLUS QU’UN STUDIO,", "UN LIEU POUR S’EXPRIMER."]} className="mt-5 text-5xl sm:text-7xl" /><p className="mt-7 max-w-2xl text-base leading-8 text-muted-foreground">{t.about.text}</p><Link to="/qui-sommes-nous" className="motion-link mt-7 inline-flex min-h-11 items-center gap-2 text-sm font-semibold uppercase hover:text-primary">{t.about.link}<ArrowRight className="size-4" /></Link></Reveal></div></section>
