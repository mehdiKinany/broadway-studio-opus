...
function EventCard({ event }: { event: StudioEvent }) { const t = useT(); return <article className="grid h-full bg-background sm:grid-cols-[minmax(0,1fr)_10rem]"> <div className="p-6 sm:p-8"><p className="text-xs font-semibold uppercase text-primary">{formatDate(event)}</p><h3 className="font-display mt-4 text-3xl sm:text-4xl">{event.titre}</h3>{event.description && <p className="mt-5 leading-7 text-muted-foreground">{event.description}</p>}<div className="mt-6 space-y-2 text-sm">{event.horaire && <p className="flex items-center gap-2"><Clock3 className="size-4 text-primary" />{event.horaire}</p>}<p className="flex items-center gap-2"><MapPin className="size-4 text-primary" />{event.lieu}</p></div></div>{event.affiche && <img src={event.affiche} alt={t.events.pastPosterAlt} className="h-full max-h-72 w-full object-contain bg-muted p-3 sm:max-h-none" loading="lazy" />}</article>; }
...
function parseDate(value: string) { const parts = value.split("-").map(Number); return new Date(Date.UTC(parts[0] ?? 1970, (parts[1] ?? 1) - 1, parts[2] ?? 1)); }
