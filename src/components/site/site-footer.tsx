...
export function SiteFooter() {
  ...
  return <footer className="bg-surface-deep pb-24 pt-16 text-hero-foreground md:pb-10 lg:pt-24">
    <div className="section-shell grid gap-12 border-b border-hero-foreground/15 pb-14 md:grid-cols-2 lg:grid-cols-[1.3fr_.8fr_1fr_1.2fr]">
      ...
      <div><h2 className="text-xs font-semibold uppercase text-primary">{t.footer.navigation}</h2><div className="mt-5 flex flex-col items-start gap-3 text-sm"><Link to="/planning" className="motion-link hover:text-primary">{t.nav.planning}</Link><Link to="/evenements" className="motion-link hover:text-primary">{t.nav.events}</Link><Link to="/gallery" className="motion-link hover:text-primary">{t.nav.gallery}</Link><Link to="/qui-sommes-nous" className="motion-link hover:text-primary">{t.nav.studio}</Link><Link to="/contact" className="motion-link hover:text-primary">{t.nav.contact}</Link></div></div>
      <div><h2 className="text-xs font-semibold uppercase text-primary">{t.footer.universes}</h2><div className="mt-5 grid justify-items-start gap-3 text-sm">{main.map((item) => <Link key={item.slug} to={item.href} className="motion-link hover:text-primary">{item.name}</Link>)}</div></div>
