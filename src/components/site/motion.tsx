...
export function Reveal({ children, className, index = 0, as: Tag = "div" }: { children: ReactNode; className?: string; index?: number; as?: ElementType }) {
  ...
  return <Tag ref={ref} className={cn("reveal-block", active && "is-reveal-ready", delayClass(index), visible && "is-visible", className)}>{children}</Tag>;
}
...
export function RevealTitle({ lines, className }: { lines: readonly string[]; className?: string }) {
  ...
  return <h2 ref={ref} className={cn("masked-title editorial-title", active && "is-reveal-ready", visible && "is-visible", className)}>{lines.map((line, index) => <span key={line} className={delayClass(index)}>{line}</span>)}</h2>;
}
...
export function RevealImage({ src, alt, className, imageClassName }: { src: string; alt: string; className?: string; imageClassName?: string }) {
  ...
  return <div ref={ref} className={cn("image-reveal overflow-hidden", active && "is-reveal-ready", visible && "is-visible", className)}><img src={src} alt={alt} className={cn("size-full object-cover", imageClassName)} /></div>;
}