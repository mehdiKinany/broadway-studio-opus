...
export function Reveal({ children, className, index = 0, as: Tag = "div" }: { children: ReactNode; className?: string; index?: number; as?: ElementType }) {
  ...
  return <Tag ref={ref} onTransitionEnd={(event: React.TransitionEvent<HTMLElement>) => { if (event.currentTarget === event.target) event.currentTarget.style.willChange = "auto"; }} className={cn("reveal-block", active && "is-reveal-ready", delayClass(index), visible && "is-visible", className)}>{children}</Tag>;
}
